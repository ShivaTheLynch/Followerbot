...
public class GWCAMemory
{
    ...
    private const int MAX_WAIT_TIME_MS = 60000; // 1 minute timeout
    
    // Add method to check if game is loaded
    public bool IsGameReady()
    {
        try
        {
            ...
            // Check for specific modules that indicate game is loaded
            // You might need to adjust these module names based on what indicates "fully loaded" for GW
            string[] requiredModules = { "Gw.exe", "d3d9.dll", "dinput8.dll" };
    ...
    // Add method to wait for game to be ready
    public async Task<bool> WaitForGameReady(IProgress<string>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        while (!IsGameReady())
        {
            ...
            {
                progress?.Report("Timeout waiting for game to load");
                return false;
            }

            progress?.Report("Waiting for game to fully load...");
            await Task.Delay(1000, cancellationToken);
        }

        progress?.Report("Game is ready");
        return true;
    }

    // Modify LoadModule to wait for game to be ready
    public async Task<LoadModuleResult> LoadModuleAsync(
        string modulepath, 
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            progress?.Report("Waiting for game to be ready...");
            if (!await WaitForGameReady(progress, cancellationToken))
            {
                progress?.Report("Game failed to load in time");
                return LoadModuleResult.GAME_NOT_READY;
            }

            // Add small delay after game is ready for extra safety
            await Task.Delay(2000, cancellationToken);

            progress?.Report($"Injecting {Path.GetFileName(modulepath)}...");
            return LoadModule(modulepath);
        }
        catch (OperationCanceledException)
        {
            progress?.Report("Operation cancelled");
            return LoadModuleResult.CANCELLED;
        }
        catch (Exception ex)
        {
            progress?.Report($"Error: {ex.Message}");
            return LoadModuleResult.ERROR;
        }
    }

    // Add method for loading multiple DLLs
    public async Task<List<(string DllPath, LoadModuleResult Result)>> LoadModulesAsync(
        IEnumerable<string> modulePaths,
        IProgress<string>? progress = null,
        CancellationToken cancellationToken = default)
    {
        var results = new List<(string DllPath, LoadModuleResult Result)>();

        // Wait for game to be ready first
        if (!await WaitForGameReady(progress, cancellationToken))
        {
            return modulePaths.Select(p => (p, LoadModuleResult.GAME_NOT_READY)).ToList();
        }

        // Add small delay after game is ready
        await Task.Delay(2000, cancellationToken);

        // Load each DLL with a small delay between them
        foreach (var path in modulePaths)
        {
            if (cancellationToken.IsCancellationRequested)
                break;

            progress?.Report($"Injecting {Path.GetFileName(path)}...");
            var result = LoadModule(path);
            results.Add((path, result));

            if (result != LoadModuleResult.SUCCESSFUL)
            {
                progress?.Report($"Failed to inject {Path.GetFileName(path)}");
                continue;
            }

            progress?.Report($"Successfully injected {Path.GetFileName(path)}");
            await Task.Delay(1000, cancellationToken); // Small delay between DLLs
        }
