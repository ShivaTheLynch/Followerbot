...
# Create a black brush for the background
...
class GWLauncher:
    def __init__(self):
        ...
        self.accounts_file = os.path.join(self.app_directory, "accounts.encrypted")
        self.salt_file = os.path.join(self.app_directory, "salt.key")
        
        # Set up encryption after paths are defined
        ...
        self.ID_SELECT_TOOLBOX = 1007
        self.ID_TOGGLE_TOOLBOX = 1008
    ...
    def setup_encryption(self):
        ...
        try:
            ...
            # Create password dialog window class
            wc = win32gui.WNDCLASS()
            wc.lpfnWndProc = self.password_dialog_proc
            wc.lpszClassName = "PasswordDialog"
            wc.hbrBackground = win32gui.GetStockObject(win32con.WHITE_BRUSH)
            
            try:
                win32gui.UnregisterClass("PasswordDialog", None)
            except:
                pass
            ...
            self.pwd_dialog = win32gui.CreateWindow(
                wc.lpszClassName,
            ...
            self.pwd_input = win32gui.CreateWindow(
                "EDIT", "",
                win32con.WS_CHILD | win32con.WS_VISIBLE | win32con.WS_BORDER | win32con.ES_PASSWORD,
                20, 20, 260, 25,
                self.pwd_dialog, 0, 0, None
            )
            
            # Create OK button
            self.pwd_ok = win32gui.CreateWindow(
                "BUTTON", "OK",
                win32con.WS_CHILD | win32con.WS_VISIBLE | win32con.BS_PUSHBUTTON,
                110, 60, 80, 30,
                self.pwd_dialog, 1, 0, None
            )
            
            # Show dialog and wait for result
            win32gui.ShowWindow(self.pwd_dialog, win32con.SW_SHOW)
            
            # Create an event to wait for password
            self.password_event = win32event.CreateEvent(None, 0, 0, None)
            self.master_password = None
            
            # Wait for password input
            win32event.WaitForSingleObject(self.password_event, win32event.INFINITE)
    ...
    def password_dialog_proc(self, hwnd, msg, wparam, lparam):
        if msg == win32con.WM_COMMAND:
            if wparam == 1:  # OK button
                # Get password from input
                length = win32gui.SendMessage(self.pwd_input, win32con.WM_GETTEXTLENGTH, 0, 0) + 1
                buffer = win32gui.PyMakeBuffer(length)
                win32gui.SendMessage(self.pwd_input, win32con.WM_GETTEXT, length, buffer)
                self.master_password = buffer[:-1].decode('utf-16')
                
                # Signal password received
                win32event.SetEvent(self.password_event)
    ...
    def encrypt_data(self, data):
        ...
        try:
            ...
            self.debug_window.log(f"Encryption error: {str(e)}")
            return None
    ...
    def decrypt_data(self, encrypted_data):
        ...
        try:
            ...
            self.debug_window.log(f"Decryption error: {str(e)}")
            return None
        ...
        try:
            ...
            try:
                ...
                for account in self.accounts:
                    ...
                    purple_brush = win32gui.CreateSolidBrush(win32api.RGB(128, 0, 128))  # Purple color
                    old_brush = win32gui.SelectObject(memdc, purple_brush)
                    win32gui.Rectangle(memdc, button_x, y, button_x + button_width, y + 25)
                    win32gui.SelectObject(memdc, old_brush)
                    win32gui.DeleteObject(purple_brush)
                    
                    # Set text color to white for Edit button
                    win32gui.SetBkMode(memdc, win32con.TRANSPARENT)
                    win32gui.SetTextColor(memdc, 0x00FFFFFF)
                ...
                # Create larger font for bottom buttons
                logfont = win32gui.LOGFONT()
                logfont.lfHeight = 20
                logfont.lfFaceName = "Arial"
                font = win32gui.CreateFontIndirect(logfont)
            ...
            if os.path.exists(self.accounts_file):
                with open(self.accounts_file, "rb") as f:
                    ...
                    if not encrypted_data:
                        self.debug_window.log("accounts file is empty")
                        self.accounts = []
                        return
                    
                    decrypted_data = self.decrypt_data(encrypted_data)
                    if decrypted_data is None:
                        self.debug_window.log("Failed to decrypt accounts data")
                        self.accounts = []
                        return
                    
                    self.accounts = []
                    for acc_data in decrypted_data:
                        acc = Account(
                ...
                self.debug_window.log("accounts file does not exist")
                self.accounts = []
        except Exception as e:
            self.debug_window.log(f"Error loading accounts: {str(e)}")
            self.accounts = []
    ...
    def save_accounts(self):
        ...
        try:
            ...
            # Encrypt and save
            encrypted_data = self.encrypt_data(accounts_data)
            if encrypted_data is None:
                raise Exception("Failed to encrypt accounts data")
    ...
    def inject_toolbox(self, pid):
        ...
        self.debug_window.log(f"Injecting GWToolbox from: {self.toolbox_path}")
                ...
                def injection_thread():
                    try:
                        ...
                        if self.wait_for_gw_window(pid):
                            ...
                            # First inject GWToolboxdll.dll
                            self.debug_window.log("Attempting GWToolboxdll.dll injection...")
                            if self.inject_toolbox(pid):
                                self.debug_window.log("GWToolboxdll.dll injection successful")
                                
                                # Wait 30 seconds before injecting the other DLL
                                self.debug_window.log("Waiting 30 seconds before injecting configured DLL...")
                                time.sleep(30)
                                
                                # Verify process is still running before second injection
                                try:
                                    process = psutil.Process(pid)
                                    if process.status() == psutil.STATUS_RUNNING:
                                        self.debug_window.log("Attempting configured DLL injection...")
                                        if self.inject_dll(pid):
                                            self.debug_window.log("Configured DLL injection successful")
                                        else:
                                            self.debug_window.log("Configured DLL injection failed")
                                    else:
                                        self.debug_window.log("Process no longer running, skipping configured DLL injection")
                                except psutil.NoSuchProcess:
                                    self.debug_window.log("Process no longer exists, skipping configured DLL injection")
                            else:
        ...
        if msg == win32con.WM_COMMAND:
            ...
            if id == 1001:  # Browse button
                ...
                try:
                    ...
                    self.debug_window.log(f"Error saving dialog data: {str(e)}")
    ...
    def save_dialog_data(self):
        ...
        for field, hwnd in self.controls.items():
            if win32gui.IsWindow(hwnd):  # Check if control still exists
                if field == "elevated":
        ...
        if missing_fields:
            ...
            if win32gui.IsWindow(self.dialog_hwnd):  # Check if dialog still exists
