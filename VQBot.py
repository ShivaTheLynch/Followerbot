...
def IsCurrentPathFinished():
    global FSM_vars
    if FSM_vars.current_map_pathing is None or len(FSM_vars.current_map_pathing.path) == 0:
        return True
...
FSM_vars.path_to_farm_machine.AddState(name="Check For Pathing Map",
                        ...
                        exit_condition=lambda: FSM_vars.current_map_id != 0 and FSM_vars.current_map_pathing is not None,
                        run_once=False)
...
FSM_vars.path_to_farm_machine.AddSubroutine(name="Handle Map Pathing",
                       ...
                       condition_fn=lambda: IsCurrentPathFinished() or Map.GetMapID() != FSM_vars.current_map_id)
...
def ResetEnvironment():
    ...
    FSM_vars.current_map_id = 0
    FSM_vars.current_map_pathing = None
    FSM_vars.current_target = None
    FSM_vars.current_loot_target = None
    FSM_vars.current_chest_target = 0
