...
def CheckMapLocation():
    ...
    if Routines.Transition.IsExplorableLoaded():
        ...
        if current_map_id not in map_paths:
            Routines.Transition.TravelToOutpost(bot_vars.starting_map)
            SetPendingAction(2000)
            return
        
        Py4GW.Console.Log(bot_vars.window_module.module_name, f"{Map.GetMapName(current_map_id)} ({current_map_id}) loaded. Initializing farm path.", Py4GW.Console.MessageType.Info)
...
def Resign():
    ...
    # Add check to prevent resignation right after blessing
    if FSM_vars.state_machine.get_previous_step_name() == "Take Bounty":
        FSM_vars.state_machine.jump_to_state_by_name("Start Pathing for Farm")
        return
...
def IsCurrentPathFinished():
    ...
    try:
        ...
        if not FSM_vars.current_map_pathing or not FSM_vars.movement_handler:
            Py4GW.Console.Log(bot_vars.window_module.module_name, "No valid path or movement handler", Py4GW.Console.MessageType.Warning)
            return False
            
        # Add check for current map ID
        if Map.GetMapID() != FSM_vars.current_map_id:
            return False
            
        # Check if we're at the end of the path
        is_path_finished = Routines.Movement.IsFollowPathFinished(FSM_vars.current_map_pathing, FSM_vars.movement_handler)
        
        # Only consider the path finished if we're not currently fighting and we've actually started moving
        if is_path_finished and not EnemyFound() and FSM_vars.movement_handler.has_started():
            # Reset movement handler to ensure clean state
            FSM_vars.movement_handler.reset()
            Py4GW.Console.Log(bot_vars.window_module.module_name, "Path is genuinely finished", Py4GW.Console.MessageType.Info)
            return True
...
FSM_vars.state_machine.AddState(name="Resign",
                       ...
                       exit_condition=lambda: bot_vars.resign_to_farm and FSM_vars.has_resigned,
                       run_once=False)
FSM_vars.state_machine.AddState(name="Wait if no resign",
                       exit_condition=lambda: bot_vars.resign_to_farm,
                       run_once=False)
...
def Take_Bounty():
    ...
    try:
        ...
        if current_map_id in map_paths:
            Py4GW.Console.Log(bot_vars.window_module.module_name, f"Found path for map {current_map_id}", Py4GW.Console.MessageType.Info)
            # Reset states
            FSM_vars.has_resigned = False
            FSM_vars.finished_resigning = False
            FSM_vars.movement_handler.reset()
            
            # Initialize path and map ID
            FSM_vars.current_map_id = current_map_id
            FSM_vars.current_map_pathing = Routines.Movement.PathHandler(map_paths[current_map_id])
            Py4GW.Console.Log(bot_vars.window_module.module_name, f"Path initialized with {len(map_paths[current_map_id])} points", Py4GW.Console.MessageType.Info)
            
            # Reset the farm machine state before transitioning
